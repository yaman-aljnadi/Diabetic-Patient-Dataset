...
CFG <- list(
  data_path = "C:/Users/anabi/Downloads/diabetic_data_project.csv",
  response  = "readmitted",
  seed      = 123,
  out_dir   = "outputs",
  save_plots = TRUE,
...
cat("Number of predictors (All features, after NZV drop):", ncol(X_train_all), "\n")
# After Box-Cox + Center/Scale + Correlation pruning
cat("Number of predictors (Box-Cox + Center/Scale + Correlation-pruned):", ncol(X_train_bc), "\n")
...
fda_grid <- expand.grid(
  degree = c(1, 2),     # polynomial degree
  nprune = seq(10, 20, 1)  # number of terms to prune, adjust as needed
)
m_fda <- train(
  x_train, y_train,
  method = "fda",
...
# # NDA Confusion Matrix
# conf_nda <- conf_list[["NDA"]]
# print(conf_nda)
# imp_nda <- varImp(m_nda)
# plot(imp_nda, top = 5)
# print(imp_nda)

# Nnet Confusion Matrix
conf_nnet <- conf_list[["Neural Network"]]
print(conf_nnet)
imp_nnet <- varImp(m_nnet)
print(imp_nnet)

# FDA Confusion Matrix
conf_fda <- conf_list[["FDA"]]
print(conf_fda)
